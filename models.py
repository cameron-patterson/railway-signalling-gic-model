...
def model(section_name, axle_pos_a=None, axle_pos_b=None, ex_blocks=None, ey_blocks=None, ex_uniform=None, ey_uniform=None, electrical_staggering=True, **kwargs):
    ...
    yg_trac_nodal_a[1:-1] = half_yg_trac_a[:-1] + half_yg_trac_a[1:]  # Each middle traction rail node will have a contribution of half of the preceding sub block's leakage and half of the next. We kept the values halved above, so now just add instead of halving again
    ...
    y_matrix = np.zeros((n_nodes, n_nodes))  # Set default nodal admittance matrix to size, filled with zeroes
    # Use stamps to add to different indices of the nodal admittance matrix
    stamp_series_admittance(y_matrix, node_locs_trac_a[:-1], node_locs_trac_a[1:], ye_trac_a)  # Traction rail series admittances
    stamp_series_admittance(y_matrix, node_locs_sig_a[:-1], node_locs_sig_a[1:], ye_sig_a)  # Signal rail series admittances
    stamp_series_admittance(y_matrix, node_locs_relay_trac_a, node_locs_relay_sig_a, y_relay)  # Relay series admittances
    stamp_series_admittance(y_matrix, node_locs_power_trac_a, node_locs_power_sig_a, y_power)  # Power supply series admittances
    stamp_series_admittance(y_matrix, node_locs_axle_trac_a, node_locs_axle_sig_a, y_axle)  # Axle series admittances
    stamp_parallel_admittance(y_matrix, node_locs_trac_a, yg_trac_nodal_a)  # Traction rail parallel admittances
    stamp_parallel_admittance(y_matrix, node_locs_sig_a, yg_sig_nodal_a)  # Signal rail parallel admittances
    # Repeat for opposite track
    stamp_series_admittance(y_matrix, node_locs_trac_b[:-1], node_locs_trac_b[1:], ye_trac_b)
    stamp_series_admittance(y_matrix, node_locs_sig_b[:-1], node_locs_sig_b[1:], ye_sig_b)
    stamp_series_admittance(y_matrix, node_locs_relay_trac_b, node_locs_relay_sig_b, y_relay)
    stamp_series_admittance(y_matrix, node_locs_power_trac_b, node_locs_power_sig_b, y_power)
    stamp_series_admittance(y_matrix, node_locs_axle_trac_b, node_locs_axle_sig_b, y_axle)
    stamp_parallel_admittance(y_matrix, node_locs_trac_b, yg_trac_nodal_b)
    stamp_parallel_admittance(y_matrix, node_locs_sig_b, yg_sig_nodal_b)
    # Note: Cross bonds only need to be added once as they connect both tracks
    stamp_series_admittance(y_matrix, node_locs_cb_a, node_locs_cb_b, y_cb)  # Cross bond admittances
    ...
    j_matrix = np.zeros([n_nodes, len(ex_blocks[0, :])])  # Set default current matrix to size, filled with zeroes

    # Use stamps to add to different indices of the current matrix
    stamp_current_source(j_matrix, node_locs_trac_a[:-1], node_locs_trac_a[1:], i_trac_sub_block_a)  # Fill current sources along traction rail
    stamp_current_source(j_matrix, node_locs_sig_a[:-1], node_locs_sig_a[1:], i_sig_sub_block_a)  # Fill current sources along signal rail
    ...
    if electrical_staggering:
        stamp_current_source(j_matrix, node_locs_power_trac_a[0::2], node_locs_power_sig_a[0::2], i_power)  # Fill half of power supply current sources
        stamp_current_source(j_matrix, node_locs_power_trac_a[1::2], node_locs_power_sig_a[1::2], -i_power)  # Fill other half of power supply current sources
    else:
        stamp_current_source(j_matrix, node_locs_power_trac_a, node_locs_power_sig_a, i_power)
    # Repeat for opposite track
    stamp_current_source(j_matrix, node_locs_trac_b[:-1], node_locs_trac_b[1:], i_trac_sub_block_b)
    stamp_current_source(j_matrix, node_locs_sig_b[:-1], node_locs_sig_b[1:], i_sig_sub_block_b)
    if electrical_staggering:
        stamp_current_source(j_matrix, node_locs_power_trac_b[0::2], node_locs_power_sig_b[0::2], i_power)
        stamp_current_source(j_matrix, node_locs_power_trac_b[1::2], node_locs_power_sig_b[1::2], -i_power)
    else:
        stamp_current_source(j_matrix, node_locs_power_trac_b, node_locs_power_sig_b, i_power)
